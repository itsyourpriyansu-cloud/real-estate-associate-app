...
- ~~`MockUserRepository.getCurrent()` returns the first ASSOCIATE regardless of who signed in~~ — **fixed** in the org-hierarchy sub-phase 1 pass (2026-09-24): it now resolves by the session's phone. A phone that matches no seeded user still falls back to the demo associate when no session phone is set at all (e.g. a test that never signs in) — see `currentUserOrNull` in `repositories/mock/effects.ts`.
...
| 2026-09-24 | Org-hierarchy sub-phase 1 (Team/Role real entities, User extended) → next agent | First of a 9-sub-phase plan (`docs/AGENT_HANDOFF.md` links the full plan; see the approved plan file referenced in the conversation that started this work) implementing the org-hierarchy master spec's Phases 1-4. **This sub-phase only**: `src/domain/role.ts` (new — `OrganizationLevel` fixed enum CEO/MANAGEMENT/MARKETING_HEAD/SENIOR_ASSOCIATE/JUNIOR_ASSOCIATE, ~40-string `Permission` enum, dynamic stored `Role`) + `src/domain/permissions.ts` (`hasPermission`/`hasAllPermissions`/`hasAnyPermission`, pure). `Team` (`src/domain/team.ts`) is now a real stored entity (`id, name, marketingHeadId, status`), not a free-text label. `User` (`src/domain/user.ts`) gained `roleId`, `orgLevel`, `teamId`, `statusHistory` (append-only, `status` now `ACTIVE\|INACTIVE\|SUSPENDED`); dropped `role`/`teamName`; `sponsorId` renamed `reportingManagerId` and is now hierarchy-validated (`ALLOWED_MANAGER_LEVELS` + `requireValidHierarchy` in `repositories/mock/effects.ts`) — a Junior Associate can never manage anyone, a Senior Associate only Junior Associates, etc. **`UserRepository.getCurrent()`'s long-standing bug is fixed**: it used to always return the first seeded `ASSOCIATE` regardless of who signed in; it now resolves by the session's phone (`MockContext.currentPhone()`/`setCurrentPhone()`, instance state — deliberately *not* module-level, so isolated test repositories never leak a phone into each other; bridged from `authStore` via `repositories.setCurrentSessionPhone`, prototype-only). `SalesTarget.teamName` → `teamId`. Seed rebuilt: 3 teams (`YHIPL2`/`YHIPL1`/new `YHIPL3`), 1 CEO + 1 Management + 3 Marketing Heads added on top of the existing ~18 associates (2 new Junior Associates added to give `YHIPL3` a body); several existing sponsor links were re-pointed (not just renamed) to satisfy the strict hierarchy — e.g. a Senior Associate can no longer report to a peer Senior Associate — so the demo associate's downline shrank from 9 (multi-level) to 4 (flat, all direct Junior Associate reports); this is an intentional org-chart correction, not a regression, and `flow.test.ts`/`screens.test.tsx`/`vara-components.test.tsx` were updated to the new numbers. `admin.ts`/`AdminRepository`/`TeamRepository` are all **untouched** in this sub-phase (Admin→User merge is sub-phase 2, flagged as the highest-risk step); `MockAdminRepository`/`MockTeamRepository`/`MockSalesRepository`/`MockSummaryRepository` were mechanically updated for the new field names/gates only (`orgLevel` checks replace `role`/designation checks where they were load-bearing; `designation`-string checks that were purely cosmetic display were left alone since seed keeps `designation` in sync with `orgLevel`). New `RepositoryErrorCode: 'FORBIDDEN'` (unused by any repository yet — lands with `requirePermission` in sub-phase 3). `__tests__/permissions.test.ts` new. `npm run validate` green (typecheck clean, lint clean, **479/479 tests pass**). Full plan (all 9 sub-phases, deviations, risk flags) is in the plan file the user approved before this work started — read it before continuing; sub-phase 2 (Admin→User merge, session-kind collapse to `none\|guest\|staff`) is next. |
