...
| `Admin` (`admin.ts`)        | `id, fullName, phone, email?`. Deliberately not a `User` — no `associateCode`/`sponsorId`/`teamName`/`designation`; structurally like `guest` (a session kind with no backing `User` row)                                                                                       |
| `AssociateIncentive` (`incentive.ts`) | `id, associateId, commissionRate (0–1), rewardPlotTarget (positive int), assignedByAdminId, assignedAt`. Set by an admin (`AdminRepository.assignIncentive`), scoped **per associate** (not per team, unlike `SalesTarget`). Read by the associate as `PendingIncentive` (`{state:'READY',value}` or `{state:'PENDING'}`) via `SalesRepository.getIncentive()` |
